...
export default function Textform(props) {
  ...
  return (
    ...
    <h1 className='mb-4'>{props.heading}</h1>
       <div className="mb-3">
       <textarea className="form-control" id="myBox" onChange={handleOnchange} value={text} style={{backgroundColor : props.mode === 'dark'?'#13466e':'white',color : props.mode==='dark'?'white':'#042743'}} rows="7"></textarea>
       </div>
      ...
      <div className="container my-3" style={{color : props.mode === 'dark'?'white':'black'}}>
        ...
        <p>{0.008 * text.split(/\s+/).filter((element)=>{return element.length!==0}).length} Mimutes to read</p>
        <h2>Preview</h2>
